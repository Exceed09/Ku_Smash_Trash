...
export async function getAllBinStatus() {
    const response = await axios.get(`https://ecourse.cpe.ku.ac.th/exceed09/all_bins/status`)
...
export async function getResetAverageInZone() {
    const response = await axios.get(`https://ecourse.cpe.ku.ac.th/exceed09/zone/reset_data`)
    return response.data.message[0]
}

export async function getBinById(id) {
    const response = await axios.get(`https://ecourse.cpe.ku.ac.th/exceed09/all_bins/${id}/status`)
    return response.data
}
